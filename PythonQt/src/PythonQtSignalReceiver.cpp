...
void PythonQtSignalReceiver::disconnectAndRemoveSignalHandlers()
{
    int disconnectsDone = 0;
    foreach(PythonQtSignalTarget target, _targets)
    {
        if (QMetaObject::disconnect(_obj, target.signalId(), this, target.slotId()))
            disconnectsDone++;
    }
    qDebug() << "PythonQt: " << disconnectsDone << " disconnected signals from " << _objClassInfo->className();
